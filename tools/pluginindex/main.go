...
func buildIndex(root string) ([]byte, error) {
	...
	// Third-party forks override the marketplace display name via
	// PLUGININDEX_NAME; upstream defaults to the official source name.
	indexName := os.Getenv("PLUGININDEX_NAME")
	if indexName == "" {
		indexName = "New API Official Plugins"
	}
	index := indexFile{IndexVersion: 1, Name: indexName, Plugins: make([]indexPlugin, 0, len(byKey))}
